 ...
 *
 * **This creates no admin.** Granting somebody the run of every account on the
 * platform is a different act from loading sample data, and it belongs to a
 * different decision — see `seed-admin.ts`. Bundling the two would mean every
 * environment that wanted the demo figures also got an operator account with a
 * published password, including the shared ones where that is exactly wrong.
 */
...
async function main() {
  ...
  console.log("For the admin console, run: npm run seed:admin");
}
